...
> **v0.2 架构**：填写层为**浏览器插件**（MV3）——以插件为「手」、本地服务为「脑」：页面上能规则匹配的字段零 LLM 直接写入，冷门问法由 AI 仅做「字段语义映射」，档案值永不出本机。原 Playwright 外驱路线保留为传统模式。当前版本 **v0.2.13**，版本演进见 [CHANGELOG.md](CHANGELOG.md)。
