...
![AutoOffer 桌面端界面 — 投递列表](assets/screenshots/app-overview.png)

*桌面端一览：左侧功能导航（档案中心 / 模型配置 / 任务 / 投递列表 / 回放），投递台账自带累计投递、状态分布与近 7 天趋势看板，每条投递记录本次填写的成功/失败/待确认统计。*
...
| 🧠 两段式匹配引擎 | 第一段本地规则直填（零 LLM、零数据外发）→ 未命中字段走映射三级漏斗：映射缓存（同档案同字段零 LLM）→ 别名词典/归一化精确匹配（40 余组常见问法）→ LLM 兜底（**只见标签不见值**）+ AI 选选项（级联逐层下钻） |
| 🧩 通用控件引擎 | 日历/月份面板/省市区级联/搜索型下拉/组合行/「至今」开关按**行为与内容特征**识别，不为单个网站写适配——华为 AUI、北森 Phoenix、Moka、飞书、智联等自研组件库实测通过；整页 iframe 内嵌表单逐帧填报 |
...
| 📮 投递台账 | 填写完成自动登记（公司/岗位自动识别、同 URL 去重），状态跟踪：已填写/已提交/面试中/已拒/放弃；看板统计累计投递、状态分布、近 7 天趋势，每条记录保存本次填写统计 |
...
**真站实测**：华为、海格通信（北森 zhiye）、满帮/长虹（Moka）、星网（starnet）、飞书招聘、大华、游卡、柏楚电子、奇瑞、达梦数据库等真实申请页面。
...
- **工程**：ruff + mypy、pytest（后端 268 项）+ 前端 vitest（32 项）、GitHub Actions
...
scripts/     打包 / 测试资产脚本
tests/       后端 268 项测试：单元 + 集成 + 插件引擎真实 Chromium 基准页
